...
export const Route = createFileRoute("/_authenticated/classes/$id/configs/$configId/runs/$runId/distributions/$distId/present")({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [dist, rows, students] = await Promise.all([
        ...
        supabase.from("run_distribution_groups").select("group_index, student_id").eq("distribution_id", distId),
    ...
    <div className="fixed inset-0 z-50 overflow-auto bg-background p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <div className="text-sm text-muted-foreground">Distribution #{data.dist.rank} · Score {data.dist.score}</div>
            <h1 className="text-3xl font-semibold">Groups</h1>
          </div>
          <Button variant="outline" onClick={() => navigate({ to: "/classes/$id/configs/$configId/runs/$runId", params: { id, configId, runId } })}>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {groups.map((g, gi) => (
            <div key={gi} className="rounded-xl border-2 border-border bg-card p-6 shadow-sm">
              <div className="mb-3 text-sm font-medium uppercase tracking-wider text-muted-foreground">Group {gi + 1}</div>
              <ul className="space-y-2">
                {g.map((sid) => (
                  <li key={sid} className="text-xl font-medium">{nameById.get(sid) ?? sid}</li>
                ))}
              </ul>
            </div>
          ))}
